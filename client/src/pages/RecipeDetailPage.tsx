  ...
  if (error || !recipe) {
    return (
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <p className="text-sm" style={{ color: '#ef4444' }}>{error || 'Recipe not found'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-10">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-6 text-sm" style={{ color: theme.textMuted }}>
          {recipe.totalTimeMinutes > 0 && (
            <span><strong>Total Time:</strong> {recipe.totalTimeMinutes} mins</span>
          )}
          {recipe.servings && (
            <>
              <span className="hidden sm:inline opacity-30">|</span>
              <span><strong>Yield:</strong> {recipe.servings} servings</span>
            </>
          )}
          {recipe.difficulty && (
            <>
              <span className="hidden sm:inline opacity-30">|</span>
              <span className="capitalize"><strong>Difficulty:</strong> {recipe.difficulty}</span>
            </>
          )}
          {recipe.cuisine && (
            <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        ...
        <section className="md:col-span-1">
          <h2 className="text-xl font-bold mb-6 pb-2 border-b" style={{ color: theme.text, borderColor: theme.border }}>
            Ingredients
          </h2>
          {recipe.ingredients?.length ? (
            ...
            <p className="text-sm" style={{ color: theme.textMuted }}>No ingredients listed.</p>
          )}
        </section>
        ...
        <section className="md:col-span-2">
          ...
          <div className="space-y-8">
            {recipe.instructions?.length ? (
                ...
                .map((s: any, idx: number) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {s.timerMinutes > 0 && (
                        <StepTimer minutes={s.timerMinutes} theme={theme} />
                      )}
                    </div>
              ...
              <p className="text-sm" style={{ color: theme.textMuted }}>No instructions listed.</p>
            )}
          </div>
        </section>
      </div>
      {/* Actions Footer */}
      <div className="mt-16 pt-8 border-t flex justify-center gap-4" style={{ borderColor: theme.border }}>
        <button
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/40 backdrop-blur-md">
          <div className="w-full max-w-sm p-8 rounded-xl text-center" style={{ background: theme.card, border: `1px solid ${theme.border}` }}>
            <h2 className="text-xl font-medium mb-2" style={{ color: theme.text }}>Delete Recipe?</h2>
            <p className="text-sm mb-8" style={{ color: theme.textMuted }}>This action cannot be undone.</p>
            <div className="flex flex-col gap-3">
              <button onClick={handleDelete} className="w-full py-3 rounded-lg font-bold text-white bg-red-500">
                Delete Forever
              </button>
              <button onClick={() => setShowDeleteModal(false)} className="w-full py-3 rounded-lg font-medium" style={{ color: theme.text }}>
                Cancel
              </button>
            </div>
          </div>
