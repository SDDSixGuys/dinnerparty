...
export class FolderService {
  ...
  async create(data: Record<string, unknown>, userId: string) {
    ...
    if (data.parentId) {
      ...
      parentPath = parent.path === "/" ? `/${parent._id}` : `${parent.path}/${parent._id}`;
      depth = parent.depth + 1;
    }

    const folder = await this.folderRepository.create({ ...data, path: parentPath, depth }, userId);

    // Update path to include own ID
    const fullPath = parentPath === "/" ? `/${folder._id}` : `${parentPath}/${folder._id}`;
  ...
  async move(id: string, userId: string, newParentId: string | null) {
    ...
    const newPath = newParentPath === "/" ? `/${folder._id}` : `${newParentPath}/${folder._id}`;
    const depthDiff = newDepth - folder.depth;

    // Update all descendants' paths
    const descendants = await this.folderRepository.findDescendants(`${oldPath}/`, userId);

    for (const descendant of descendants) {
      const updatedPath = descendant.path.replace(oldPath, newPath);
      await this.folderRepository.update(String(descendant._id), userId, {
        path: updatedPath,
  ...
  async delete(id: string, userId: string) {
    ...
    await Recipe.updateMany({ userId, folderIds: id }, { $pull: { folderIds: id } });
